...
import { Save, Building2, Phone, Mail, MapPin, Instagram, Globe, Sparkles, KeyRound, ShieldCheck, Check, Lock, RotateCcw } from 'lucide-react';
...
export const GeneralSettingsTab: React.FC = () => {
  ...
  useEffect(() => {
    if (siteData?.general) {
      setFormData(siteData.general);
    }
  }, [siteData?.general]);

  const handleRestoreRcDefaults = () => {
    setFormData(initialSiteData.general);
    updateGeneralConfig(initialSiteData.general);
    showToast('Dados oficiais da RC Engenharia Solar carregados com sucesso!', 'success');
  };
        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Edite todos os dados da empresa exibidos no topo, formulários e rodapé.
            </p>
          </div>
          <div className="flex items-center gap-2.5">
            <button
              type="button"
              onClick={handleRestoreRcDefaults}
              className="px-3.5 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-xs flex items-center gap-1.5 transition-colors cursor-pointer border border-slate-300/60 dark:border-slate-700"
              title="Carrega os dados oficiais da RC Engenharia Solar"
            >
              <RotateCcw className="w-3.5 h-3.5 text-amber-500" />
              <span>Restaurar Padrão RC Solar</span>
            </button>
            <button
              type="submit"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="https://www.instagram.com/seu_perfil (opcional)"
              className="w-full p-2.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-sm"
            />
            <p className="text-[11px] text-slate-500">
              Insira o link oficial do perfil da sua empresa. Se deixado em branco, o ícone de Instagram será ocultado do cabeçalho e rodapé.
            </p>
          </div>
