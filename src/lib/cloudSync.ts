...
// Global Cloud Sync Channel identifier for Globo Solar Energia
const CLOUD_SYNC_APP_ID = 'globo_solar_energia_prod_v1';
const STORAGE_SYNC_KEY = 'globo_solar_cloud_sync_meta';
...
const CLOUD_STORAGE_PROVIDERS = [
  ...
  // Provider 2: Backup Cloud Storage endpoint
  {
    name: 'jsonblob',
    save: async (payload: CloudPayload): Promise<boolean> => {
      let blobId = localStorage.getItem('globo_solar_blob_id');
      try {
        if (!blobId) {
          ...
          if (res.ok) {
            ...
            if (location) {
              ...
              localStorage.setItem('globo_solar_blob_id', blobId);
              return true;
            }
          }
        } else {
    ...
    load: async (): Promise<CloudPayload | null> => {
      const blobId = localStorage.getItem('globo_solar_blob_id');
...
export async function pushToCloudSync(siteData: FullSiteData, leads: LeadItem[]): Promise<boolean> {
  ...
  try {
    if (typeof BroadcastChannel !== 'undefined') {
      const bc = new BroadcastChannel('globo_solar_sync_channel');
