...
const STORAGE_SITE_DATA_KEY = 'globo_solar_site_data_v1';
const STORAGE_LEADS_KEY = 'globo_solar_leads_v1';
const STORAGE_THEME_KEY = 'globo_solar_theme_mode';
const ADMIN_AUTH_KEY = 'globo_solar_admin_auth';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const syncWithSupabase = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!settingsErr && settingsRow) {
        setSiteData((prev) => ({
          ...prev,
          general: settingsRow.general || prev.general,
  ...
  useEffect(() => {
    ...
    try {
      if (typeof BroadcastChannel !== 'undefined') {
        broadcastChannel = new BroadcastChannel('globo_solar_sync_channel');
        broadcastChannel.onmessage = (event) => {
          if (event.data && event.data.siteData) {
            setSiteData(event.data.siteData);
    ...
    if (client) {
      ...
      supabaseChannel = client
        .channel('globo_solar_realtime_sync')
  ...
  const resetToDefaultData = useCallback(() => {
    ...
    showToast('Conteúdo restaurado para o padrão original da Globo Solar.', 'info');
  }, [showToast]);
